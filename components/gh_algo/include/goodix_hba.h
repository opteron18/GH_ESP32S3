...
typedef enum {
	...
	HBA_SCENES_BIKING_INSIDE = 10,		//???????ùù?
	HBA_SCENES_BIKING_OUTSIDE = 11,		//???????ùù?
	HBA_SCENES_BIKING_MOUNTAIN= 12,		//???????ùù???
...
typedef struct {
    ...
    uint32_t hba_convergence_rate;					// ???????ùù?????????
    uint32_t senseless_mode_step;                   // ??ùù????????????0???ùù?
    uint32_t senseless_mode_duration;               // ??ùù?????????????0???ùù?
}goodix_hba_config;
...
typedef struct
{
	...
	int32_t *cur_adj_flg;		// ??????????ùù
	int32_t *gain_adj_flg;		// ?????????ùù
	int32_t *enable_flg;		// ????????ùù
...
typedef struct
{
	uint32_t hba_out_flag;				// ???????? 1 ??ùù
	int32_t hba_out;					// ?????
	float32_t hba_confi;				// ?????
	float32_t hba_snr;					// ?????   : ????????????????????????????????
    uint32_t valid_level;			    // ?????? : 0 -> 1 -> 2 ?????????
    uint32_t valid_score;				// ??????? : 0->100 ???????
    uint32_t hba_acc_info;              // ????? : 0-?????ùù?????1-????-???????2-???-???????
    hba_scenes_e hba_acc_scence;            // ??????? : ?ùù? hba_scenes_e ???????
	// ??ùù????????????????
	uint32_t *hba_out_flag_channel;				// ???????? 1 ??ùù
	uint32_t *hba_out_channel;					// ?????
	float32_t *hba_confi_channel;				// ?????
	float32_t *hba_snr_channel;					// ?????
	float32_t **p_hba_td_filter;				// ????????????????
	float32_t **p_hba_fd_filter;			// ???(???)????????????ù4ùù??1ùù????
}goodix_hba_result;
...
* @brief ???HBA?????????ùù??
* @param[in/out]  ver       ?ùù?????????
* @param[in] 	  ver_len   ?ùù?????????????HBA_INTERFACE_VERSION_LEN_MAX
* @return     	  ??
*/
