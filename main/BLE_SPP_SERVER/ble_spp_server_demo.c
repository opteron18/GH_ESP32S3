...
#define SAMPLE_DEVICE_NAME          "Linka_Device"    //The Device Name Characteristics in GAP
...
static const uint8_t spp_adv_data[21] = {
    /* Flags */
    0x02,0x01,0x06,
    /* Complete List of 16-bit Service Class UUIDs */
    0x03,0x03,0xF0,0xAB,
    /* Complete Local Name in advertising */
    0x0D,0x09, 'L', 'I', 'N', 'K', 'A', '_', 'D', 'E', 'V', 'I', 'C','E'
};
...
void uart_task(void *pvParameters)
{
    ...
    for (;;) {
        ...
        if (xQueueReceive(spp_uart_queue, (void * )&event, (TickType_t)portMAX_DELAY)) {
            ...
            case UART_DATA:
                // ESP_LOGI(GATTS_TABLE_TAG,"EVENT.SIZE: %d",event.size);
...
#endif
#ifdef CONFIG_EXAMPLE_SPP_THROUGHPUT
                        //  ESP_LOGI(GATTS_TABLE_TAG,"event.size <= (spp_mtu_size - 3)");
    	...
    	case ESP_GATTS_WRITE_EVT: {
            ...
            if (p_data->write.is_prep == false) {
                if (res == SPP_IDX_SPP_COMMAND_VAL) {
                    ...
                    ESP_LOGI(GATTS_TABLE_TAG,"value: %s",p_data->write.value);
                    ...
                    // 调用协议处理函数处理接收到的BLE数据
                    int result = handle_ble_write_event(p_data);
                    if (result == 0) {
                        ESP_LOGI(GATTS_TABLE_TAG, "协议数据处理成功");
                    } else {
                        ESP_LOGE(GATTS_TABLE_TAG, "协议数据处理失败");
                    }
#endif
                } else {
                    //TODO:
...
void send_data_pack_thread(void *pvParameters){
